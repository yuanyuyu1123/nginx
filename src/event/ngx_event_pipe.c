 ...
 */
        ...
        for (;;) {
            ...
            } else if (!p->cacheable && !p->writing && p->in) { //说明数据时缓存到内存中的
                cl = p->in;
...
*/
    ...
    ngx_http_write_filter中会把新来的数据加到r->out后面,也就是未发送的数据在r->out前面新数据在链后面,所以实际write是之前未发送的先发送出去*/
        ...
        for (cl = p->free; cl; cl = cl->next) {
            ...
            if (cl->buf->last_shadow) {  //配合参考ngx_http_fastcgi_input_filter阅读
...
ngx_int_t
ngx_event_pipe_add_free_buf(ngx_event_pipe_t *p, ngx_buf_t *b) {  //将参数的b代表的数据块挂入free_raw_bufs的开头或者第二个位置.b为上层觉得没用了的数据块.
    ngx_chain_t *cl;
