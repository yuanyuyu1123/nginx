...
{//下面的解析以://比如rewrite  ^(/xyz/aa.*)$   http://$http_host/aa.mp4   break;为例
    ...
    if (ngx_strncmp(value[2].data, "http://", sizeof("http://") - 1) == 0
        ...
        || ngx_strncmp(value[2].data, "$scheme", sizeof("$scheme") - 1) == 0) { //nginx判断,如果是用http://等开头的rewrite,就代表是垮域重定向.会做302处理.
...
ngx_http_rewrite_set(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    if (v->get_handler == NULL) {  //如果变量名称不是以上开头,则其get_handler为ngx_http_rewrite_var,data为index .
