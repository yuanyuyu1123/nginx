...
*/
static ngx_int_t
ngx_http_fastcgi_handler(ngx_http_request_t *r) { //FCGI处理入口,ngx_http_core_run_phases里面当做一个内容处理模块调用的.(NGX_HTTP_CONTENT_PHASE阶段执行),实际赋值在:
...
ngx_http_fastcgi_create_key(ngx_http_request_t *r) { //根据之前在解析scgi_cache_key指令的时候计算出来的复杂表达式结构,存放在flcf->cache_key中的,计算出cache_key.
    ngx_str_t *key;
    ngx_http_fastcgi_loc_conf_t *flcf;
...
ngx_http_fastcgi_create_request(ngx_http_request_t *r) { //ngx_http_upstream_init_request中执行该函数
    ...
    size = sizeof(ngx_http_fastcgi_header_t) //#1
           ...
           + len + padding
           + sizeof(ngx_http_fastcgi_header_t)  //#4 /* NGX_HTTP_FASTCGI_PARAMS */

           + sizeof(ngx_http_fastcgi_header_t); //#5 /* NGX_HTTP_FASTCGI_STDIN */
    ...
    if (params->lengths) { //处理FCGI的参数,进行相关的拷贝操作.
        ...
        while (*(uintptr_t *) le.ip) { //获取对应的变量参数字符串
            ...
            for (val_len = 0; *(uintptr_t *) le.ip; val_len += lcode(&le)) {  //也就是取出fastcgi_param  SCRIPT_FILENAME  xxx;中字符串xxx的字符串
                lcode = *(ngx_http_script_len_code_pt *) le.ip; //为什么这里解析到一个参数和值后会退出for呢?因为在ngx_http_fastcgi_init_params中在value对应的code后面添加了一个NULL空指针,也就是下面的le.ip += sizeof(uintptr_t);
            }
            le.ip += sizeof(uintptr_t);
...
ngx_http_fastcgi_process_header(ngx_http_request_t *r) { //解析FCGI的请求返回记录,如果是返回标准输出,则解析其请求的HTTP头部并回调其头部数据的回调.数据部分还没有解析.
...
//ngx_http_parse_header_line函数碰到了\r\n\r\n于是返回NGX_HTTP_PARSE_HEADER_DONE,然后本函数就执行完成.
    ...
    for (;;) { //启用cache的情况下,注意这时候buf的内容实际上已经在ngx_http_upstream_process_header中出去了为缓存如文件中预留的头部内存
        ...
        if (f->state < ngx_http_fastcgi_st_data) { //上次的状态都没有读完一个头部,先解析这些头部看看是不是有问题.
            ...
            if (f->type == NGX_HTTP_FASTCGI_STDOUT && f->length == 0) { //接收到的是携带数据的fastcgi标识,单length却为0,
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                ...
                if (flcf->catch_stderr) {
                    ...
                    for (i = 0; i < flcf->catch_stderr->nelts; i++) {
                        if (ngx_strnstr(msg, (char *) pattern[i].data,
                                        p - msg)
                            != NULL) { //fastcgi_catch_stderr "XXX";中的xxx和p-msg信息中匹配,则返回invalid,该函数返回后然后请求下一个后端服务器
                            return NGX_HTTP_UPSTREAM_INVALID_HEADER;
                        }
                    }
                }
...
ngx_http_fastcgi_input_filter_init(void *data) { //主要功能就是解析fastcgi格式包体,解析出包体后,把对应的buf加入到p->in
    //当解析完带有\r\n\r\n的头部的FCGI包后,后面的包解析都由这个函数进行处理.
    ngx_http_request_t *r = data;
...
ngx_http_fastcgi_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module); //获取当前的location,即在哪个location配置的"fastcgi_pass"指令
