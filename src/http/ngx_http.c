...
//赋值未ngx_http_request_body_save_filter
...
/*解析HTTP配置的流程:
    ...
    1)图4-1中的主循环是指Nginx进程的主循环,主循环只有调用配置文件解析器才能解析nginx.conf文件(这里的"主循环"是指解析全部配置文件的循环代码,
    图8-6的第4步,为了便于理解,可以认为是Nginx框架代码在循环解析配置项).
    2)当发现配置文件中含有http{}关键字时,HTTP框架开始启动,这一过程详见10.7节描述的ngx_http_block方法.
    3) HTTP框架会初始化所有HTTP模块的序列号,并创建3个数组用于存储所有HTTP模块的create_main_conf、create_srv_conf、create_loc_conf方法返回的指针地址,
    并把这3个数组的地址保存到ngx_http_conf_ctx_t结构中.
    ...
    8) HTTP框架开始循环解析nginx.conf文件中http{．．．}里面的所有配置项,过程到第19步才会返回.
    9)配置文件解析器在检测到1个配置项后,会遍历所有的HTTP模块,ngx_command_t数组中的name项是否与配置项名相同.
    10)如果找到有1个HTTP模块(如mytest模块)对这个配置项感兴趣(如test_myconfig配置项),就调用ngx_command_t结构中的set方法来处理.
    11) set方法返回是否处理成功.如果处理失败,那么Nginx进程会停止.
    12)配置文件解析器继续检测配置项.如果发现server{．．．)配置项,就会调用ngx_http_core_module模块来处理.
    因为ngx_http_core_module模块明确表示希望处理server{}块下的配置项.注意,这次调用到第18步才会返回.
    13) ngx_http_core_module棋块在解析server{...}之前,也会如第3步一样建立ngx_http_conf_ctx_t结构,并建立数组保存所有HTTP模块返回的指针地址.
    然后,它会调用每个HTTP模块的create_srv_conf、create_loc_conf方法(如果实现的话).
    ...
    20)在第3步和第13步,以及我们没有列幽来的某些步骤中(如发现其他server块或者location块),都创建了ngx_http_conf_ctx_t结构,
    这时将开始调用merge_srv_conf、merge_loc_conf等方法合并这些不同块(http、server、location)中每个HTTP模块分配的数据结构.
...
//图形化参考:深入理解NGINX中的图9-2,图10-1,图4-2
{
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ...
    ngx_http_max_module = ngx_count_modules(cf->cycle, NGX_HTTP_MODULE); //二级类型按照在ngx_modules中的顺序排序
   ...
   出现的顺序保存在http core模块的loc配置的locations队列中,上面的代码中先对每个server的location进行排序和分类处理,这一步发生在 ngx_http_init_location()函数中*/
...
NGX_HTTP_ACCESS_PHASE、NGX_HTTP_CONTENT_PHASE、NGX_HTTP_LOG_PHASE)是允许任何一个HTTP模块实现自己的ngx_http_handler_pt处理方法,
并将其加入到这7个阶段中去的.在调用HTTP模块的postconfiguration方法向这7个阶段中添加处理方法前,
需要先将phases数组中这7个阶段里的handlers动态数组初始化(ngx_array_t类型需要执行ngx_array_init方法初始化),
在这一步骤中,遁过调用ngx_http_init_phases方法来初始化这7个动态数组.
    ...
    for (i = 0; i < NGX_HTTP_LOG_PHASE; i++) {
        ...
        switch (i) { //所有阶段的checker在ngx_http_core_run_phases中调用
            ...
            case NGX_HTTP_REWRITE_PHASE:
                ...
                //该阶段则不允许HTTP模块加入自己的ngx_http_handler_pt方法处理用户请求方法,
        ...
        for (j = cmcf->phases[i].handlers.nelts - 1; j >= 0; j--) {
            ...
            ph->handler = h[j]; //i阶段的所有ngx_http_handler_pt处理方法通过ph连接起来,也就是全部添加到cmcf->phase_engine.handlers数组中,各个成员通过ph->next连接在一起
                       ...
                       ngx_http_module_t *module, ngx_uint_t ctx_index)
{
     ...
     * cmcf在外层赋值为ctx->main_conf,所以为http{}内,server{}外的server配置项, saved.srv_conf为ctx->srv_conf,所以为server{}内的server配置项,例如:
    http {
        aaa;
        server {
            aaa
...
{
    ngx_uint_t                   n;
    ngx_queue_t                 *q, *locations, *named, tail; //最后name noname regex都会连接到tail链表中
    ngx_http_core_loc_conf_t    *clcf;
    ngx_http_location_queue_t   *lq;
...
在开始的这个locationqueue中,有一些是有相同路径前缀的,自然他们在排序的时候也是会挨在一起的,
ngx_http_create_locations_list会将跟某个location有相同前缀的location,从原来的queue上取下,挂在该location的list成员下,
这样在主queue上的location都是具有相互不同name的location了.举个例子:
                    ...
                    ngx_http_listen_opt_t *lsopt)
{
    ...
    for (i = 0; i < cmcf->ports->nelts; i++) {
        ...
        /*不通IP,相同端口:相同端口的存储在同一个ngx_http_conf_port_t结构中,不同的IP地址存到ngx_http_conf_port_t->addrs成员数组中*/
    ...
    return ngx_http_add_address(cf, cscf, port, lsopt); //把解析到的所有listen地址端口信息添加
}
...
#if (NGX_HAVE_INET6)
            case AF_INET6:
                if (ngx_http_add_addrs6(cf, hport, addr) != NGX_OK) { //后面有addr++,所以这里的addr对应的是addr[i]的地址
                    return NGX_ERROR;
                }
                break;
#endif
...
中会监听80端口worker次,那么子进程创建起来后,不是每个字进程都关注这worker多个 listen事件了吗?
为了避免这个问题,nginx通过在子进程运行ngx_event_process_init函数的时候,通过ngx_add_event来控制子进程关注的listen,最终实现只关注master进程中创建的一个listen事件*/
