...
/*HTTP 1.0及HTTP0.9规定浏览器与服务器只保持短暂的连接,浏览器的每次请求都需要与服务器建立一个TCP连接,服务器完成请求处理后立即断开TCP连接,服务器不跟踪每个客户也不记录过去的请求.*/
...
typedef struct {  //获取请求对应的server块信息,server_name信息可以参考ngx_http_set_virtual_server作为例子
    ...
    ngx_chain_t *free;
    /* 可以通过listen xxx ssl或者ssl on启用ssl */
...
struct ngx_http_request_s { //当接收到客户端请求数据后,调用ngx_http_create_request中创建并赋值
   ...
   /* ngx_http_internal_redirect置1,创建子请求的时候,子请求也要置1,见ngx_http_subrequest,所有子请求需要做重定向
    ...
    /*为1时表示发送给客户端的HTTP响应头部已经发送.在调用ngx_http_send_header方法后,若已经成功地启动响应头部发送流程,
