...
ngx_http_init_connection(ngx_connection_t *c) {
    ...
    ngx_add_timer(rev, cscf->client_header_timeout);  //把接收事件添加到定时器中,当post_accept_timeout秒还没有客户端数据到来,就关闭连接
...
ngx_http_request_handler(ngx_event_t *ev) {
     ...
     * 方法中已经将write_event_handler设置为ngx_http_core_run_phases方法,而一般我们开发的不太复杂的HTTP模块是不会重新设置write_event_handler方法的,
     * 因此,一旦有可写事件时,就会继续按照流程执行ngx_http_core_run_phases方法,并继续按阶段调用各个HTTP模块实现的方法处理请求.
...
ngx_http_post_request(ngx_http_request_t *r, ngx_http_posted_request_t *pr) {  //注意是把新创建的ngx_http_posted_request_t添加到最上层r的posted_requests中(例如现在是第四层r,则最上层是第一次r,而不是第三层r)
...
void
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc) { //subrequest注意ngx_http_run_posted_requests与ngx_http_postpone_filter ngx_http_finalize_request配合阅读
    ngx_connection_t *c;
...
ngx_http_finalize_connection方法在结束请求时,解决了keepalive特性和子请求的问题ngx_http_finalize_request -> ngx_http_finalize_connection,
 注意和ngx_http_terminate_request的区别*/
...
ngx_http_finalize_connection(ngx_http_request_t *r) { //ngx_http_finalize_request->ngx_http_finalize_connection
    ...
    if (!ngx_terminate
        && !ngx_exiting
        && r->keepalive
        && clcf->keepalive_timeout > 0) { //如果客户端请求携带的报文头中设置了长连接,并且我们的keepalive_timeout配置项大于0(默认75s),则不能关闭连接,只有等这个时间到后还没有数据到来,才关闭连接
...
ngx_http_set_keepalive(ngx_http_request_t *r) {
    ...
    ngx_add_timer(rev, clcf->keepalive_timeout);  //启动保活定时器,时间通过keepalive_timeout设置
...
ngx_http_lingering_close_handler(ngx_event_t *rev) {
    ...
    if (rev->timedout || c->close) { //该定时器在ngx_http_set_lingering_close设置的
        ngx_http_close_request(r, 0);
        return;
    }
...
ngx_int_t
ngx_http_send_special(ngx_http_request_t *r, ngx_uint_t flags) {//该函数实际上就是做一件事,就是在要发送出去的chain链末尾添加一个空chain(见ngx_output_chain->ngx_output_chain_add_copy),当遍历chain发送数据的时候,以此来标记数据发送完毕,这是最后一个chain,
