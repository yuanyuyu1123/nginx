...
static ngx_int_t ngx_http_get_forwarded_addr_internal(ngx_http_request_t *r,
                                                      ngx_addr_t *addr, u_char *xff, size_t xfflen,
                                                      ngx_array_t *proxies,
                                                      int recursive);
...
static ngx_command_t ngx_http_core_commands[] = {
  ...
  */
        ...
        {ngx_string("location"),
         ...
         0,
         NULL},
        //listen [ip:port | ip(不知道端口默认为80) | 端口 | *:端口 | localhost:端口] [ngx_http_core_listen函数中的参数]
        {ngx_string("listen"),
...
ngx_http_send_response(ngx_http_request_t *r, ngx_uint_t status,
                       ngx_str_t *ct, ngx_http_complex_value_t *cv) { //status真正发送给客户端的头部行组包生效在ngx_http_status_lines
    ngx_int_t rc;
    ngx_str_t val;
    ngx_buf_t *b;
    ngx_chain_t out;
...
//调用ngx_http_output_filter方法即可向客户端发送HTTP响应包体,ngx_http_send_header发送响应行和响应头部
ngx_int_t
ngx_http_output_filter(ngx_http_request_t *r, ngx_chain_t *in) { //如果内容被保存到了临时文件中,则会在ngx_http_copy_filter->ngx_output_chain->ngx_output_chain_copy_buf->ngx_read_file中读取文件内容,然后发送
    ngx_int_t rc;
    ngx_connection_t *c;
...
ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy) { //图形化参考:深入理解NGINX中的图9-2  图10-1  图4-2,结合图看,并可以配合http://tech.uc.cn/?p=300看
    ...
    ngx_conf_t save;
    ngx_http_module_t *module;
    ngx_http_conf_ctx_t *ctx, *pctx;  //父级ctx
