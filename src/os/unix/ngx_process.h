...
typedef struct {
    ...
    /*这是由socketpair系统调用产生出的用于进程间通信的socket句柄,这一对socket句柄可以互相通信,目前用于master父进程与worker子进程问的通信*/
    ...
    unsigned            detached:1; //标志位,为1时表示在进行父、子进程分离.即将子进程挂在init进程下,防止父进程(shell)退出而导致子进程退出
...
#define NGX_PROCESS_JUST_RESPAWN  -4 //在ngx_spawn_process()创建worker进程时,将ngx_processes[s].just_spawn = 1,以此作为区别旧的worker进程的标记
