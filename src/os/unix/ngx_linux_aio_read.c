...
┃int io_setup(unsigned nr_events,        ┃I/O上下文可以处理的事件的最小个     ┃执行成功后ctxp就是分配的上下  ┃
┃aio context_t *ctxp)                    ┃数,ctxp是文件异步I/O的上下文描述   ┃文描述符,这个异步I/O上下文   ┃
┃                                        ┃                                    ┃将至少可以处理nr- events个事  ┃
┃                                        ┃符指针                              ┃                              ┃
┃                                        ┃                                    ┃件.返回0表示成功             ┃
┣━━━━━━━━━━━━━━━━━━━━╋━━━━━━━━━━━━━━━━━━╋━━━━━━━━━━━━━━━┫
┃                                        ┃                                    ┃  销毁文件异步I/O的上下文.   ┃
┃int io_destroy (aio_context_t ctx)      ┃  ctx是文件异步I/O的上下文描述符    ┃                              ┃
┃                                        ┃                                    ┃返回0表示成功                 ┃
┣━━━━━━━━━━━━━━━━━━━━╋━━━━━━━━━━━━━━━━━━╋━━━━━━━━━━━━━━━┫
┃int io_submit(aiio_context_t ctx,       ┃  ctx是文件异步I/O的上下文描述符,  ┃  提交文件异步I/O操作.返回   ┃
┃                                        ┃nr是一次提交的事件个数,cbp是需要   ┃                              ┃
...
ngx_file_aio_read(ngx_file_t *file, u_char *buf, size_t size, off_t offset,
                  ngx_pool_t *pool) { //注意aio内核读取完毕后,是放在ngx_output_chain_ctx_t->buf中的,见ngx_output_chain_copy_buf->ngx_file_aio_read
    ngx_err_t err;
    struct iocb *piocb[1];
