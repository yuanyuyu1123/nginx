...
那么不会使得当前进程进入睡眠状态,而是始终保持进程在可执行状态,每当内核调度到这个进程执行时就持续检查是否可以获取到锁.
在拿不到锁时,这个进程的代码将会一直在自旋锁代码处执行,直到其他进程释放了锁且当前进程获取到了锁后,代码才会继续向下执行.
...
自旋锁实际上是忙等锁
    当锁不可用时,CPU一直循环执行"测试并设置"该锁直到可用而取得该锁,CPU在等待自旋锁时不做任何有用的工作,仅仅是等待.
因此,只有在占用锁的时间极短的情况下,使用自旋锁才是合理的.当临界区很大或有共享设备的时候,需要较长时间占用锁,使用自旋锁会降低系统的性能.
...
进程时间片用完的时候,在切换到该等待自旋锁的时候,如果发现该锁还是不可用,则一直循环检测,直到时间片用完切换到其他进程,在自旋锁等待获取共享资源的过程中会一直占用用CPU资源.
