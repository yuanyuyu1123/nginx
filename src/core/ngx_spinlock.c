...
*/
void
ngx_spinlock(ngx_atomic_t *lock, ngx_atomic_int_t value, ngx_uint_t spin) { //如果lock为0,表示可以获取锁,然后把lock置为value,如果lock不为0,则循环调度一会儿,再次检查,如果spin次循环还没有获取到锁,则让出CPU控制权
