...
//内存池数据结构,链表形式存储,实际上只有第一个pool才有这么多字段,后面的都只有ngx_pool_data_t字段,以节约内存
struct ngx_pool_s {
    ...
    ngx_pool_large_t     *large;//节点中大内存块指针,pool中指向大数据块的指针(大数据块是指 size > max 的数据块)
