...
#define ngx_slab_slots(pool)                                                  \
    (ngx_slab_page_t *) ((u_char *) (pool) + sizeof(ngx_slab_pool_t)) //定位slot[]数组
...
#define ngx_slab_page_addr(pool, page)                                        \
    ((((page) - (pool)->pages) << ngx_pagesize_shift)                         \
     + (uintptr_t) (pool)->start) //得到实际分配的页的起始地址
...
*/
void *
ngx_slab_alloc_locked(ngx_slab_pool_t *pool, size_t size) { //这儿假设page_size是4KB
    size_t s;
    ...
    if (page->next != page) { //如果之前已经有此类大小obj且那个已经分配的内存缓存页还未满  9 个ngx_slab_page_t都还没有用完
        ...
        if (shift < ngx_slab_exact_shift) { //小obj,size < 128B,更新内存缓存页中的bitmap,并返回待分配的空间在缓存的位置
            ...
            bitmap = (uintptr_t *) ngx_slab_page_addr(pool, page); //pool->start开始的128字节为需要分配的空间,其后紧跟一个int 4字节空间用于bitmap
            /*
            ...
            for (m = (uintptr_t) 1 << NGX_SLAB_MAP_SHIFT, i = 0;
                ...
                if ((page->slab & NGX_SLAB_MAP_MASK) == mask) { //如果整个page页的所有slab已经用完,则会在后面的ngx_slab_alloc_pages从新获取空间并划分slab,然后分配
                    prev = ngx_slab_page_prev(page);
                    prev->next = page->next;
                    page->next->prev = page->prev;
    ...
    */
    if (page) {
        ...
        if (shift < ngx_slab_exact_shift) {
            bitmap = (uintptr_t *) ngx_slab_page_addr(pool, page); //page页的起始地址的一个uintptr_t类型4字节用来存储bitmap信息

            n = (ngx_pagesize >> shift) / ((1 << shift) * 8); //计算bitmap需要多少个slot obj块

            if (n == 0) { //至少需要一个4M页面大小的一个obj(2<<shift字节)来存储bitmap信息
                n = 1;
            }
...
ngx_slab_free_locked(ngx_slab_pool_t *pool, void *p) {
    ...
    switch (type) {
        ...
        case NGX_SLAB_SMALL: //slot小于ngx_slab_exact_size
            ...
            m = (uintptr_t) 1 << (n % (8 * sizeof(uintptr_t))); //求出在uintptr_t中,p的偏移,即求出在uintptr_t中的第几位
            //由于map是根据uintptr_t划分的,所以求出该块对应的uintptr_t的偏移,即求出第几个uintptr_t
            n /= 8 * sizeof(uintptr_t); //该obj所在的是那个bitmap,例如一个页64个obj,则需要2个bitmap表示这64个obj的位图
            bitmap = (uintptr_t *)
                    ((uintptr_t) p & ~((uintptr_t) ngx_pagesize - 1)); //求出p对应的page页所在位图的位置

            if (bitmap[n] & m) { //如果第m位确实为1,
                slot = shift - pool->min_shift;//找到对应的slot_m[]的元素

                if (page->next == NULL) { //如果页面的当前状态是全部已使用,就把他链入slot_m[]中
                    slots = ngx_slab_slots(pool); //定位slot_m数组
                ...
                for (i = i + 1; i < map; i++) { //查看其他uintptr_t是否都没使用
                    if (bitmap[i]) {
                        goto done;
                    }
                }
        ...
        case NGX_SLAB_BIG: //用户请求内存的大小大于ngx_slab_exact_size
            ...
            if (slab & m) { //该slab块确实正在被使用
                ...
                if (page->next == NULL) { //如果整个页面中的所有obj块都被使用,则该页page[]和slot[]没有对应关系,因此需要把页page[]和slot[]对应关系加上
                    slots = ngx_slab_slots(pool); //定位slot[]数组
        ...
        case NGX_SLAB_PAGE: //用户归还整个页面
            ...
            ngx_slab_free_pages(pool, page, size); //归还页面
...
ngx_slab_free_pages(ngx_slab_pool_t *pool, ngx_slab_page_t *page,
    ...
    if (join < pool->last) { //join不是pool中最后一个page
        ...
        if (ngx_slab_page_type(join) == NGX_SLAB_PAGE) { //和ngx_slab_alloc_pages配合阅读,
    ...
    if (pages) { //例如一次alloc 3个page,分配是page[3 4 5],则page[5]的prev指向
