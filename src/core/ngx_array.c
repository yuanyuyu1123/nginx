...
ngx_array_push(ngx_array_t *a) {
    ...
    if (a->nelts == a->nalloc) {
        ...
        if ((u_char *) a->elts + size == p->d.last //判断后面的内存是否被别人用了
            ...
            /* allocate a new array
              如果当前内存池中剩余的空间小于本次需要新增的空间,那么对ngx_array_push方法来说,会将原先动态数组的容量扩容一倍*/
...
ngx_array_push_n(ngx_array_t *a, ngx_uint_t n) {
    ...
    if (a->nelts + n > a->nalloc) {
        ...
        if ((u_char *) a->elts + a->size * a->nalloc == p->d.last
            ...
            /* allocate a new array
             * 如果参数n小于原先动态数组的容量,将会扩容一倍;如果参数n大于原先动态数组的容量,这时会分配2×n大小的空间,扩容会超过一倍*/
