...
static ngx_command_t ngx_core_commands[] = {
        ...
        //worker_processes 设置进程个数  worker_cpu_affinity 1000 0100 0010 0001
...
会直接报错退出.在main函数结尾处,在ngx_master_process_cycle函数调用之前,会close掉这个日志文件.
  */
    ...
    if (ccf->master && ngx_process == NGX_PROCESS_SINGLE) {
        ngx_process = NGX_PROCESS_MASTER;//在这里会把进程模式设置为MASTER模式
    }
...
ngx_set_environment(ngx_cycle_t *cycle, ngx_uint_t *last) {
    ...
    for (i = 0; i < ccf->env.nelts; i++) {
        ...
        for (p = ngx_os_environ; *p; p++) {
            ...
            if (ngx_strncmp(*p, var[i].data, var[i].len) == 0 //标准的key=value环境变量
                && (*p)[var[i].len] == '=') {
                env[n++] = *p;
                break;
            }
