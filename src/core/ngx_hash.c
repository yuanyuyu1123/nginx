...
ngx_hash_init(ngx_hash_init_t *hinit, ngx_hash_key_t *names, ngx_uint_t nelts) { //使用方法可以参考ngx_http_server_names
    ...
    size_t len;
    u_short *test;
    ngx_uint_t i, n, key, size, start, bucket_size; //size表示实际需要桶的个数
...
ngx_hash_wildcard_init(ngx_hash_init_t *hinit, ngx_hash_key_t *names, //参考:http://www.bkjia.com/ASPjc/905190.html //使用方法可以参考ngx_http_server_names
    ...
    size_t len, dot_len;
    ngx_uint_t i, n, dot; //n表示当前所处理的names[]数组中的第几个成员 dot:当前解析到names[i]中的第i个元素字符串的.字符串位置
...
ngx_hash_add_key(ngx_hash_keys_arrays_t *ha, ngx_str_t *key, void *value,
    ...
    u_char *p;
    ngx_str_t *name;
    ngx_uint_t i, k, n, skip, last; //skip: 1 -- ".example.com"  2 -- "*.example.com"  0 -- "www.example.*"
    ...
    if (flags & NGX_HASH_WILDCARD_KEY) {
        ...
        for (i = 0; i < key->len; i++) {
            ...
            if (key->data[i] == '.' && key->data[i + 1] == '.') { //不能出现两个连续的..，便是出错，直接返回
                return NGX_DECLINED;
            }
