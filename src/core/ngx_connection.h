...
struct ngx_listening_s { //初始化及赋值见ngx_http_add_listening,热升级nginx的时候,继承源master listen fd在ngx_set_inherited_sockets
    ...
    void *servers;  /* array of ngx_http_in_addr_t, for example */ //赋值见ngx_http_init_listening,指向ngx_http_port_t结构
    ...
    size_t post_accept_buffer_size; //见ngx_http_add_listening
    ...
    中会监听80端口worker次,那么子进程创建起来后,不是每个字进程都关注这worker多个listen事件了吗?为了避免这个问题,nginx通过
...
struct ngx_connection_s { //cycle->read_events和cycle->write_events这两个数组存放的是ngx_event_s,他们是对应的,见ngx_event_process_init
    ...
    /*内存池.一般在accept一个新连接时,会创建一个内存池,而在这个连接结束时会销毁内存池.注意,这里所说的连接是指成功建立的
