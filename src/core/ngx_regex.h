...
//ngx_http_rewrite中,rewrite aaa bbb break;配置中,aaa解析使用ngx_regex_compile_t,bbb解析使用ngx_http_script_compile_t,赋值见ngx_regex_compile
...
/*根据正则表达式到指定的字符串中进行查找和匹配,并输出匹配的结果.使用编译好的模式进行匹配,采用与Perl相似的算法,返回匹配串的偏移位置.
