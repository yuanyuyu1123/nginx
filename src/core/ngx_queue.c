...
/*便用双向链表排序的例子:
    定义一个简单的链表,并使用ngx_queue_sort方法对所有元素排序.在这个例子
...
在此例中,可以看到ngx_queue_data的用法,即呵以根据链表元素结构体TestNode中
的qEle成员地址换算出TestNode结构体变量的地址,这是面向过程的C语言编写的ngx_queue_t链表之所以能够通用化的关键.
下面来看一下ngx_queue_data的定义:
...
{
    TeetNodek eleNode;
    ngx_queue_data(q, TestNode, qEle),
    //处理当前的链表元素eleNode
}
...
该函数功能就是按照cmp比较,然后对珍格格queue进行从新排序,可以看到,这里采用的是插入排序算法,时间复杂度为O(n),整个代码非常简洁*/
void
